...
RSpec.feature 'User uses control panel products paginator', type: :feature do
  before do
    ...
    visit control_panel_products_path
  end

  scenario 'with default settings' do
    expect(page).to have_css('#control-panel-products-table tbody tr', count: 10)
  end

  scenario 'by choosing next page' do
    find('.box-body .pagination .next a').click

    expect(page).to have_css('#control-panel-products-table tbody tr', count: 2)
