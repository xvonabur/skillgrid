...
RSpec.feature 'User uses admin products paginator', type: :feature do
  before do
    @products = []
  ...
  scenario 'with default settings' do
    expect(page).to have_css('#admin-products-table tbody tr', count: 10)
  end

  scenario 'by choosing next page' do
    find('.box-body .pagination .next a').click

    expect(page).to have_css('#admin-products-table tbody tr', count: 2)
